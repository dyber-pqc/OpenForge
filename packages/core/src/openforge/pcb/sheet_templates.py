...
def _comp(refdes: str, sym: str, lib: str, value: str, x: float, y: float,
          **fields) -> Any:
    return SchComponent(
        refdes=refdes, symbol_name=sym, library=lib, value=value,
        x=float(x), y=float(y), fields=dict(fields),
    )


def _wire(x1: float, y1: float, x2: float, y2: float, net: str = "") -> Any:
    return SchWireSegment(x1=float(x1), y1=float(y1),
                          x2=float(x2), y2=float(y2), net_name=net)
...
def _pwr(net: str, x: float, y: float, is_ground: bool = False) -> Any:
    return SchPowerSymbol(net_name=net, x=float(x), y=float(y),
                          is_ground=is_ground)


def _sheet(name: str, filename: str, ports: list[Any],
           pos: tuple[float, float] = (4000, 3000)) -> Any:
    return SchSheet(name=name, filename=filename, ports=ports, position=pos)


def _port(name: str, direction: str = "bidir", side: str = "L",
          width: int = 1, net: str = "") -> Any:
    return SchPort(name=name, direction=direction, side=side,
                   width=width, net_name=net or name)
...
def linear_regulator_3v3(input_v: float = 5.0,
                         output_current_a: float = 1.0) -> tuple[Any, Any]:
    ...
    # Regulator
    sch.components.append(_comp("U1", "AMS1117-3.3", "Regulator_Linear",
                                "AMS1117-3.3", 2000, 1800))
...
def usb_c_power_only() -> tuple[Any, Any]:
    ...
    sch.components.append(_comp("J1", "USB_C_Receptacle_USB2.0",
                                "Connector", "USB-C", 2000, 2000))
    ...
    sh = _sheet("USB_C_PWR", "usb_c_pwr.sch", [
...
def usb_c_data() -> tuple[Any, Any]:
    ...
    sch.components.append(_comp("R1", "R", "Device", "5.1k", 2800, 2200))
    sch.components.append(_comp("R2", "R", "Device", "5.1k", 2800, 2400))
    sch.components.append(_comp("U2", "USBLC6-2SC6", "Power_Protection",
                                "USBLC6", 2800, 2800))
    ...
    sh = _sheet("USB_C_DATA", "usb_c_data.sch", [
        _port("+5V", "power_out", "R"),
        _port("GND", "power_out", "B"),
...
def ftdi_ft232h_uart() -> tuple[Any, Any]:
    ...
    sch.components.append(_comp("U1", "FT232HL", "Interface_USB",
                                "FT232HL", 2500, 2000))
    for i, v in enumerate(("100nF",) * 4):
        sch.components.append(_comp(f"C{i+1}", "C", "Device", v,
                                    1500 + i * 300, 2800))
    sch.components.append(_comp("Y1", "Crystal", "Device", "12MHz",
                                3500, 2400))
...
def stm32f4_min_system() -> tuple[Any, Any]:
    ...
    sch.components.append(_comp("U1", "STM32F401RE", "MCU_ST_STM32F4",
                                "STM32F401RET6", 3000, 2500))
    # Decoupling caps - 1 per VDD pin (4 on STM32F4)
    for i in range(4):
        sch.components.append(_comp(f"C{i+1}", "C", "Device", "100nF",
                                    1500 + i * 300, 3800))
    sch.components.append(_comp("C5", "C", "Device", "4.7uF",
                                2700, 3800))  # VDDA bulk
    # HSE crystal
    sch.components.append(_comp("Y1", "Crystal", "Device", "8MHz",
                                4500, 2300))
    sch.components.append(_comp("C6", "C", "Device", "18pF", 4700, 2100))
    sch.components.append(_comp("C7", "C", "Device", "18pF", 4700, 2500))
    # Reset button + RC
    sch.components.append(_comp("SW1", "SW_Push", "Switch", "Reset",
                                1800, 2500))
...
def rp2040_min_system() -> tuple[Any, Any]:
    ...
    sch.components.append(_comp("U1", "RP2040", "MCU_RaspberryPi",
                                "RP2040", 3000, 2500))
    sch.components.append(_comp("U2", "W25Q128JV", "Memory_Flash",
                                "W25Q128JV", 4500, 2500))
    sch.components.append(_comp("U3", "LDO_1V1", "Regulator_Linear",
                                "NCP1117-1.1", 1500, 2500))
    # Decoupling
    for i in range(8):
        sch.components.append(_comp(f"C{i+1}", "C", "Device", "100nF",
                                    1200 + i * 250, 3800))
...
def esp32_wroom_section() -> tuple[Any, Any]:
    ...
    sch.components.append(_comp("U1", "ESP32-WROOM-32", "RF_Module",
                                "ESP32-WROOM-32", 3000, 2500))
    ...
    # BOOT button
    sch.components.append(_comp("SW1", "SW_Push", "Switch", "BOOT",
                                4000, 2500))
    sch.components.append(_comp("SW2", "SW_Push", "Switch", "EN",
                                4000, 2200))
    sch.power_symbols.append(_pwr("+3V3", 3000, 1800))
    sch.power_symbols.append(_pwr("GND", 3000, 4000, True))
    sh = _sheet("ESP32", "esp32.sch", [
        _port("+3V3", "power_in", "L"),
        _port("GND", "power_in", "B"),
        _port("UART_TX", "output", "R"),
        _port("UART_RX", "input", "R"),
...
def ethernet_rj45_magjack() -> tuple[Any, Any]:
    ...
    sch.components.append(_comp("J1", "RJ45_Magjack_HX1188",
                                "Connector", "HX1188", 3000, 2500))
...
def ddr3_single_chip_layout() -> tuple[Any, Any]:
    ...
    sch.components.append(_comp("U1", "MT41K256M16", "Memory_RAM",
                                "DDR3-1600", 3000, 2500))
...
def oscillator_canned() -> tuple[Any, Any]:
    ...
    sch.components.append(_comp("X1", "Oscillator", "Oscillator",
                                "25MHz", 2500, 2500))
...
def mosfet_driver_high_side() -> tuple[Any, Any]:
    ...
    sch.components.append(_comp("Q1", "PMOS", "Device", "IRF9540",
                                2500, 2000))
    sch.components.append(_comp("Q2", "NPN", "Device", "2N3904",
                                2500, 2600))
...
def op_amp_inverting(gain: float = 10.0) -> tuple[Any, Any]:
    ...
    sch.components.append(_comp("U1", "OPAMP", "Amplifier_Operational",
                                "MCP6002", 2500, 2500))
    rf = 10_000 * gain
...
def op_amp_noninverting(gain: float = 10.0) -> tuple[Any, Any]:
    ...
    sch.power_symbols.append(_pwr("-V", 2500, 2900))
    sch.power_symbols.append(_pwr("GND", 2800, 2900, True))
    sh = _sheet("OA_NONINV", "oa_noninv.sch", [
        _port("VIN", "input", "L"),
        _port("VOUT", "output", "R"),
        _port("+V", "power_in", "T"),
        _port("-V", "power_in", "B"),
...
BUILTIN_TEMPLATES: list[SchSheetTemplate] = [
    SchSheetTemplate("ldo_3v3", "LDO 3.3V",
                     "AMS1117-3.3 linear regulator",
                     linear_regulator_3v3),
    SchSheetTemplate("usb_c_pwr", "USB-C Power",
                     "USB-C receptacle, 5V only", usb_c_power_only),
    SchSheetTemplate("usb_c_data", "USB-C Data",
                     "USB-C with data pair + ESD", usb_c_data),
    SchSheetTemplate("pwr_sw", "Power Switch",
                     "Slide switch on power rail", power_switch_slide),
    SchSheetTemplate("ft232h", "FT232H UART",
                     "FTDI FT232H USB-UART", ftdi_ft232h_uart),
    SchSheetTemplate("stm32f4", "STM32F4 Min System",
                     "STM32F4 min system", stm32f4_min_system),
    SchSheetTemplate("rp2040", "RP2040 Min System",
                     "RP2040 + flash + 12MHz XO", rp2040_min_system),
    SchSheetTemplate("esp32", "ESP32-WROOM",
                     "ESP32-WROOM-32 module", esp32_wroom_section),
    SchSheetTemplate("rj45", "Ethernet RJ45",
                     "RJ45 magjack + termination", ethernet_rj45_magjack),
    SchSheetTemplate("ddr3", "DDR3 Single Chip",
                     "DDR3 x16 + decoupling + VTT",
                     ddr3_single_chip_layout),
    SchSheetTemplate("osc", "Canned Oscillator",
                     "3-pin oscillator + decap", oscillator_canned),
    SchSheetTemplate("xtal", "Crystal",
                     "HC-49 crystal + load caps", crystal_circuit),
    SchSheetTemplate("hs_sw", "High-Side Switch",
                     "P-MOS load switch", mosfet_driver_high_side),
    SchSheetTemplate("oa_inv", "Inverting Op-Amp",
                     "Inverting op-amp", op_amp_inverting),
    SchSheetTemplate("oa_noninv", "Non-Inv Op-Amp",
                     "Non-inverting op-amp", op_amp_noninverting),
]
