...
def _chip_2pad(name: str, lib_w: float, pad_w: float, pad_h: float,
               desc: str) -> PcbFootprint:
    dx = lib_w / 2
    return PcbFootprint(
        name=name, library="openforge_smd", ref="R?", value="",
        pads=[
...
def _soic(name: str, n: int, pitch: float, span: float,
          pad_w: float, pad_h: float, body_w: float, body_l: float,
          desc: str) -> PcbFootprint:
    cx = span / 2 + pad_w / 2 + 0.25
    cy = body_l / 2 + 0.25
    return PcbFootprint(
        name=name, library="openforge_smd", ref="U?",
...
def _qfp_pads(n: int, pitch: float, span: float, pad_w: float, pad_h: float) -> list[PcbPad]:
    ...
    for i in range(per_side):
        pads.append(_smd_pad(str(2 * per_side + i + 1), span / 2, -(start + i * pitch), pad_h, pad_w))
    # top side (reverse x)
    for i in range(per_side):
        pads.append(_smd_pad(str(3 * per_side + i + 1), -(start + i * pitch), -span / 2, pad_w, pad_h))
    return pads


def _qfp(name: str, n: int, pitch: float, span: float, pad_w: float, pad_h: float,
         body: float, desc: str) -> PcbFootprint:
...
def _bga(name: str, grid: int, pitch: float, pad_d: float, body: float,
         desc: str) -> PcbFootprint:
    pads: list[PcbPad] = []
    ...
    for row in range(grid):
        for col in range(grid):
            pads.append(
                PcbPad(
                    name=str(idx), x_mm=start + col * pitch, y_mm=start + row * pitch,
                    shape="round", size_x_mm=pad_d, size_y_mm=pad_d, drill_mm=0.0,
                    layers=["F.Cu", "F.Mask"],
                )
            )
            idx += 1
    c = body / 2 + 0.25
    return PcbFootprint(
        name=name, library="openforge_bga", ref="U?",
        pads=pads,
...
def _header(name: str, rows: int, cols: int, pitch: float = 2.54) -> PcbFootprint:
    ...
    for c in range(cols):
        for r in range(rows):
            ...
            pads.append(
                PcbPad(
                    name=str(idx), x_mm=x0 + c * pitch, y_mm=y0 + r * pitch,
                    shape=shape,  # type: ignore[arg-type]
                    size_x_mm=1.7, size_y_mm=1.7, drill_mm=1.0,
                    layers=["F.Cu", "B.Cu", "F.Mask", "B.Mask"],
                )
            )
            idx += 1
    ...
    h = rows * pitch
    return PcbFootprint(
        name=name, library="openforge_conn", ref="J?",
        pads=pads,
        courtyard=[(-w / 2 - 0.25, -h / 2 - 0.25), (w / 2 + 0.25, -h / 2 - 0.25),
                   (w / 2 + 0.25, h / 2 + 0.25), (-w / 2 - 0.25, h / 2 + 0.25),
                   (-w / 2 - 0.25, -h / 2 - 0.25)],
...
FOOTPRINTS: dict[str, PcbFootprint] = {
    ...
    "USB-C_Recep": PcbFootprint(
        name="USB-C_Recep", library="openforge_conn", ref="J?",
        pads=[
            _smd_pad(str(i + 1), -3.25 + i * 0.5, -3.0, 0.30, 1.20)
            for i in range(12)
        ] + [
    ...
    "USB_Micro-B": PcbFootprint(
        name="USB_Micro-B", library="openforge_conn", ref="J?",
        pads=[
            _smd_pad(str(i + 1), -1.3 + i * 0.65, -2.25, 0.40, 1.35) for i in range(5)
        ] + [
    ...
    "SMA_Edge": PcbFootprint(
        name="SMA_Edge", library="openforge_rf", ref="J?",
        pads=[
    ...
    "SMA_Vertical": PcbFootprint(
        name="SMA_Vertical", library="openforge_rf", ref="J?",
        pads=[
