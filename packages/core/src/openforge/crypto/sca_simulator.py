...
class PowerTrace:
    ...
    def downsample(self, factor: int) -> PowerTrace:
        new_samples = [
            sum(self.samples[i : i + factor]) / factor for i in range(0, len(self.samples), factor)
        ]
        return PowerTrace(
            samples=new_samples,
...
class PowerTraceSimulator:
    ...
    def _simulate_cycle_activity(
        ...
        total = 0.0
        hd = (
            self._hamming_distance(input_bytes, prev_bytes)
            if prev_bytes
            else self._hamming_weight(input_bytes)
        )
    ...
    def simulate_trace(
        ...
        for cycle in range(num_cycles):
            ...
            if sensitive:
                rot = bytes(sensitive[(i + cycle) % len(sensitive)] for i in range(len(sensitive)))
            else:
                rot = b""
...
class TvlaAnalyzer:
    ...
    def correlation_attack(
        ...
        for k in key_candidates:
            model = [
                float(bin(hypothesized_intermediate(t.input_bytes, k)).count("1")) for t in traces
            ]
            best_for_key = 0.0
            for i in range(n_samples):
    ...
    def summary_report(self, result: TvlaResult) -> str:
        ...
        if result.has_leakage:
            lines.append("RESULT: Statistically significant first-order leakage detected.")
        else:
