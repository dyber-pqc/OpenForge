...
class PdnSynthesizerPanel(QDockWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._stripe_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        ...
        self._via_stack_edit.setToolTip("Comma-separated via cuts, e.g. 'met1->met4, met4->met5'")
    ...
    def _apply_template(self) -> None:
        ...
        self._via_stack_edit.setText(", ".join(f"{a}->{b}" for a, b in t.via_stack))
        self._regenerate()
    ...
    def _to_tcl(self, cfg: PdnConfig) -> str:  # type: ignore[name-defined]
        ...
        push("define_pdn_grid -name core_grid -voltage_domains CORE -pins {}")
        if cfg.followpins:
            push(
        ...
        for v in cfg.via_stack:
            push(f"add_pdn_connect -grid core_grid -layers {{{v.from_layer} {v.to_layer}}}")
        push("pdngen")
        return "\n".join(lines) + "\n"
