...
class MainWindow(QMainWindow):
    ...
    def _on_pnr_flow(self) -> None:
        """Run full place-and-route flow via OpenROAD in WSL2."""
        if not self._project_mgr.is_open():
            self._console.append_error("No project open. Open a project first.")
            return
        ...
        wsl_proj = to_wsl(proj_path)
        wsl_pdk = to_wsl(pdk_lib_dir)
...
write_verilog {wsl_proj}/pnr_build/{top}_placed.v

puts ""
puts "=== OpenROAD P&R Complete ==="
puts "Design: {top}"
puts "PDK: SKY130"
puts "DEF: pnr_build/{top}_placed.def"
...
"""
        ...
        # Run OpenROAD via WSL2
        import os
        wsl_tcl = to_wsl(tcl_script)
        ...
        try:
            ...
            if result.returncode == 0:
                self._console.append_success("Place & Route completed successfully!")
                # Check for DEF output
                def_file = pnr_dir / f"{top}_placed.def"
                if def_file.exists():
                    self._console.append_info(f"DEF output: {def_file}")
                    self._console.append_info(f"DEF size: {def_file.stat().st_size} bytes")
            else:
                self._console.append_error(f"OpenROAD exited with code {result.returncode}")
            ...
            self._console.append_error("P&R timed out after 300s")
        except Exception as exc:
            self._console.append_error(f"P&R error: {exc}")
        finally:
