// =====================================================================
// SLH-DSA-SHA2-128s (NIST FIPS 205 / SPHINCS+) - AXI4-Lite wrapper
// ---------------------------------------------------------------------
...
module sphincs_plus_top #(
    ...
    // 0x0008: VERSION - 32'h0205_0128 (FIPS 205, 128s)
    // 0x000C: IRQ_EN
    // 0x0010: MSG_LEN
    // 0x0014: HYPERTREE_LAYER (current FORS/WOTS layer)
    // 0x0020-0x003F: PUBLIC_KEY (32 bytes)
    // 0x0040-0x007F: SECRET_KEY (64 bytes)
    // 0x0080-0x107F: MESSAGE (4096 bytes max)
    // 0x1080-0x2EBF: SIGNATURE (7856 bytes)
    // -----------------------------------------------------------------
    ...
    localparam ADDR_HT_LAYER   = 14'h0014;
    localparam ADDR_PUBLIC_KEY = 14'h0020;
    localparam ADDR_SECRET_KEY = 14'h0040;
    localparam ADDR_MESSAGE    = 14'h0080;
    localparam ADDR_SIGNATURE  = 14'h1080;
    ...
    reg [31:0] ht_layer_reg;

    initial begin
        version_reg  = 32'h0205_0128;
        ctrl_reg     = 32'h0;
        status_reg   = 32'h0;
        irq_en_reg   = 32'h0;
        msg_len_reg  = 32'h0;
    ...
    reg [31:0] pk_mem  [0:7];
    reg [31:0] sk_mem  [0:15];
    reg [31:0] msg_mem [0:31];
    reg [31:0] sig_mem [0:31];

    integer i;
    initial begin
        for (i = 0; i < 8; i = i + 1)  pk_mem[i]  = 32'h0;
        for (i = 0; i < 16; i = i + 1) sk_mem[i]  = 32'h0;
    ...
    // FSM
    // -----------------------------------------------------------------
    // SPHINCS+ has many sub-operations: WOTS+, FORS, hypertree.
    localparam STATE_IDLE     = 4'd0;
    localparam STATE_KEYGEN   = 4'd1;
    localparam STATE_FORS     = 4'd2;
    localparam STATE_WOTS     = 4'd3;
    localparam STATE_HT       = 4'd4;
    ...
    reg [3:0]  ht_layer_idx;
    localparam HT_LAYERS = 4'd7;

    // SPHINCS+ 128s is dominated by hash computations - many cycles.
    localparam CYCLES_KEYGEN = 32'd1_000_000;
    localparam CYCLES_FORS   = 32'd200_000;
    localparam CYCLES_WOTS   = 32'd80_000;
    localparam CYCLES_VERIFY = 32'd150_000;
        ...
        if (!axi_aresetn) begin
            ...
            case (state)
                STATE_IDLE: begin
                    ...
                    else if (ctrl_reg[1]) state <= STATE_FORS;
                ...
                STATE_FORS: begin
                    status_reg[0] <= 1'b1;
                    cycle_counter <= cycle_counter + 1;
                    if (cycle_counter >= CYCLES_FORS) begin
                        cycle_counter <= 32'h0;
                        state         <= STATE_WOTS;
                    end
                end
                STATE_WOTS: begin
                    cycle_counter <= cycle_counter + 1;
                    if (cycle_counter >= CYCLES_WOTS) begin
                        cycle_counter <= 32'h0;
                        state         <= STATE_HT;
                    end
                end
                STATE_HT: begin
                    cycle_counter <= cycle_counter + 1;
                    ht_layer_reg  <= {28'h0, ht_layer_idx};
                    if (cycle_counter >= CYCLES_WOTS) begin
                        cycle_counter <= 32'h0;
                        if (ht_layer_idx >= HT_LAYERS - 1) begin
                            state <= STATE_DONE;
                        end else begin
                            ht_layer_idx <= ht_layer_idx + 1;
                            state        <= STATE_WOTS;
                        end
                    end
                end
                ...
                case (aw_addr_reg)
                    ...
                    default: begin
                        if (aw_addr_reg >= ADDR_PUBLIC_KEY && aw_addr_reg < ADDR_SECRET_KEY)
                            pk_mem[((aw_addr_reg - ADDR_PUBLIC_KEY) >> 2) & 3'h7] <= wdata;
                        else if (aw_addr_reg >= ADDR_SECRET_KEY && aw_addr_reg < ADDR_MESSAGE)
                            sk_mem[((aw_addr_reg - ADDR_SECRET_KEY) >> 2) & 4'hF] <= wdata;
        ...
        if (!axi_aresetn) begin
            ...
            if (arready) begin
                ...
                case (ar_addr_reg)
                    ...
                    ADDR_HT_LAYER: rdata <= ht_layer_reg;
                    default: begin
                        if (ar_addr_reg >= ADDR_PUBLIC_KEY && ar_addr_reg < ADDR_SECRET_KEY)
                            rdata <= pk_mem[((ar_addr_reg - ADDR_PUBLIC_KEY) >> 2) & 3'h7];
                        else if (ar_addr_reg >= ADDR_SECRET_KEY && ar_addr_reg < ADDR_MESSAGE)
                            rdata <= sk_mem[((ar_addr_reg - ADDR_SECRET_KEY) >> 2) & 4'hF];
