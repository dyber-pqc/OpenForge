...
VL_INLINE_OPT void Vcounter___024root___ico_sequent__TOP__0(Vcounter___024root* vlSelf) {
    ...
    auto &vlSelfRef = std::ref(*vlSelf).get();
    // Body
    vlSelfRef.overflow = ((0xffU == (IData)(vlSelfRef.count)) 
...
#endif
            VL_FATAL_MT("src/counter.v", 4, "", "Input combinational region did not converge.");
        }
...
#endif
            VL_FATAL_MT("src/counter.v", 4, "", "NBA region did not converge.");
        }
...
#endif
                VL_FATAL_MT("src/counter.v", 4, "", "Active region did not converge.");
            }
